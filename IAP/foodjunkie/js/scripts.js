		...
		$.ajax({
			...
			success: function(msg){
				...
				if (result[0] === "success") {
					...
					errorMessage("white","Your order has been Placed. We will contact you when it is ready. Thank you for using food junkie", "green", '8000');
...
function sendComment(){
	...
	$.ajax({
		type: "POST",
		url: phpUrl,
		data: {ppointer:pointer, pname:name, pemail:email, pcomment:comment},
		success: function(msg){
			console.log(msg);
			var result = msg.split(".");
			if (result[0] === "success") {
